    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex flex-col space-y-2">
            ...
            <input 
                type="date" 
                id="endDate" 
                name="fecha_finalizacion"  <!-- Ensure this is 'fecha_finalizacion' -->
                class="px-4 py-2 w-full bg-white rounded-md border-b-2 border-slate-900 text-neutral-700"
            >
        </div>
    </div>
            ...
            <div class="flex flex-col items-center mt-6 w-full">
                ...
                >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <input id="fecha_inicio" name="fecha_inicio" type="date" class="px-4 py-2 w-full bg-blue-100 rounded-md border-b-2 border-slate-900 text-neutral-700" required />
                        </div>

                        <div class="flex flex-col space-y-2">
                            <label for="eduEndDate" class="text-xl font-bold text-slate-900">Fecha de Finalización</label>
                            <input id="fecha_finalizacion" name="fecha_fin" type="date" class="px-4 py-2 w-full bg-blue-100 rounded-md border-b-2 border-slate-900 text-neutral-700" />
                        </div>
                    </div>

                    <div class="flex flex-col space-y-2 mt-4">
                        <label for="eduDescription" class="text-xl font-bold text-slate-900">Descripción</label>
                        <textarea id="descripcion" name="descripcion" rows="4" class="px-4 py-2 w-full bg-blue-100 rounded-md border-b-2 border-slate-900 text-neutral-700 resize-none"></textarea>
                    </div>
                ...
                <div class="mt-4 w-full max-w-[985px] bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="flex gap-2">
                            <button onclick="editEducation({{ $edu->id }})"class="p-2 bg-blue-100 rounded-md hover:bg-blue-200">
                                <!-- Icono de editar -->
                            </button>
...
function editEducation(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            form.querySelector('#institucion').value = data.education.institucion;
            form.querySelector('#titulo_grado').value = data.education.titulo_grado;
            form.querySelector('#campo_estudio').value = data.education.campo_estudio;
            form.querySelector('#fecha_inicio').value = data.education.fecha_inicio;
            form.querySelector('#fecha_finalizacion').value = data.education.fecha_finalizacion || '';
            form.querySelector('#descripcion').value = data.education.descripcion || '';
            
            // Update form action for update
            form.action = `/education/update/${id}`;
            ...
            if (submitBtn) {
                submitBtn.textContent = 'Actualizar formación';
            }
        }
    })
    .catch(error => {
...
function deleteExperience(id) {
    ...
    const submitBtn = form.querySelector('button[type="submit"]');
    // Get CSRF token from the form's hidden input instead of meta tag
    ...
    .then(data => {
        if (data.success) {
            ...
            form.querySelector('input[name="_action"]').value = 'update';
...
document.getElementById('educationForm').addEventListener('submit', function(e) {
    ...
    // Get the current form action to determine if it's an update or create
    const isUpdate = this.action.includes('/education/update/');
    const url = isUpdate ? this.action : '/education';
    ...
    .catch(error => {
        console.error('Error:', error);
        alert('Error al guardar la formación académica');
    });
});
...
document.getElementById('educationForm').addEventListener('submit', function(e) {
    ...
    return false; // Add this line to prevent further handling
});
